...
def wind_drift_factor_from_trajectory(trajectory_dict, min_period=None):
    ...
    if min_period is None:
        ...
        s = np.round(min_period.total_seconds() / (timestep).total_seconds()).astype(
            int
        )
        ind = np.arange(0, len(time), s).astype(np.int32)
        print(
            "Original timestep (%s) multiplied by %i: %s" % (timestep, s, timestep * s)
        )
        ind2 = ind.copy()
        for i in range(1, s):
    ...
    lonf, latf, back_az = geod.fwd(
        lon[0:-1], lat[0:-1], current_azimuth[0:-1], current_speed[0:-1] * time_step
    )
    ...
    azimuth_offset = (
        azimuth_forward - wind_azimuth[0:-1]
    )  # 0 if downwind drift, positive if rightwards drift is needed towards end position
    azimuth_offset = (
        azimuth_offset % 360
    )  # Make sure azimuth angle is between -180 and 180
...
def skillscore_liu_weissberg(
    lon_obs, lat_obs, lon_model, lat_model, tolerance_threshold=1
):
...
def plot_wind_drift_factor(wdf, azimuth, wmax_plot=None):
    ...
    wmax = wdf.max()
    wbins = np.arange(0, wmax + 0.005, 0.005)
    ...
    plt.text(np.pi, wmax * 0.5, " Wind direction", fontsize=18)
    plt.grid()
...
def oil_wave_entrainment_rate_li2017(
    dynamic_viscosity,
    oil_density,
    interfacial_tension,
    significant_wave_height=None,
    wave_breaking_fraction=None,
    wind_speed=None,
    sea_water_density=1028.0,
):
    ...
    if significant_wave_height is None:
        ...
        significant_wave_height = significant_wave_height_from_wind_neumann_pierson(
            wind_speed
        )
    g = 9.81
...
def verticaldiffusivity_Sundby1983(
    windspeed, depth, mixedlayerdepth=50, background_diffusivity=0
):
    ...
    K[depth > mixedlayerdepth - 1] = (
        K[depth > mixedlayerdepth - 1] + background_diffusivity
    ) / 2  # Transition
...
def verticaldiffusivity_Large1994(
    windspeed, depth, mixedlayerdepth=50, background_diffusivity=0
):
    ...
    K = (
        MLD * stabilityfunction(depth / MLD) * 0.4 * G(depth / MLD) * windstress
        + (depth / MLD) * background_diffusivity
    )
...
def verticaldiffusivity_stepfunction(depth, MLD=20, k_above=0.1, k_below=0.02):
...
def gls_tke(
    windstress, depth, sea_water_density, tke, generic_length_scale, gls_parameters=None
):
    """From LADIM model."""
    ...
    phi = 100.0 * (windstress / sea_water_density) ** (3.0 / 2.0)

    # dissipation and turbulent length scale for interiour of mixed layer
    eps = (
        cmu0 ** (3.0 + p / n)
        * tke ** (3.0 / 2.0 + m / n)
        * generic_length_scale ** (-1.0 / n)
    )
    l_i = c_i * tke ** (3.0 / 2.0) * eps ** (-1.0)

    # diffusivity for interior of mixed layer
    # c_i = sqrt(2.) * cmu0**3
    ki = c_i * (2.0 * tke) ** 0.5 * l_i
...
def plot_stokes_profile(
    profiles, view=["vertical", "birdseye", "u", "v"], filename=None
):
...
def stokes_transport_monochromatic(mean_wave_period, significant_wave_height):
    mean_wave_frequency = 2.0 * np.pi / mean_wave_period
    return mean_wave_frequency * np.power(significant_wave_height, 2) / 16


def stokes_drift_profile_monochromatic(
    stokes_u_surface, stokes_v_surface, significant_wave_height, mean_wave_period, z
):
    """
...
def stokes_drift_profile_exponential(
    stokes_u_surface, stokes_v_surface, significant_wave_height, mean_wave_period, z
):
    """
    Breivik, O., Janssen, P., Bidlot, J., 2014. Approximate stokes drift profiles in deep water.
...
def stokes_drift_profile_phillips(
    stokes_u_surface, stokes_v_surface, significant_wave_height, mean_wave_period, z
):
    """
    ...
    km = (
        ...
        * (1 - 2 * beta / 3)
        / (
            2
            * stokes_transport_monochromatic(mean_wave_period, significant_wave_height)
        )
    )

    stokes_speed = stokes_surface_speed * (
        np.exp(2 * km * z)
        - beta
        * np.sqrt(2 * np.pi * km * np.abs(z))
        * sp.special.erfc(np.sqrt(2 * km * np.abs(z)))
    )
...
def stokes_drift_profile_windsea_swell(
    stokes_u_surface,
    stokes_v_surface,
    swell_mean_direction_to,
    swell_mean_period,
    swell_height,
    wind_sea_mean_direction_to,
    wind_sea_mean_period,
    wind_sea_height,
    z,
):
    """
    ...
    stokes_swell_surface_speed = (
        stokes_u_surface * th_ws_N - stokes_v_surface * th_ws_E
    ...
    stokes_swell_u, stokes_swell_v, stokes_swell_speed = (
        stokes_drift_profile_monochromatic(
            stokes_swell_surface_u,
            stokes_swell_surface_v,
            swell_height,
            swell_mean_period,
            z,
        )
    ...
    stokes_wind_u, stokes_wind_v, stokes_wind_speed = stokes_drift_profile_phillips(
        stokes_wind_surface_u,
        stokes_wind_surface_v,
        wind_sea_height,
        wind_sea_mean_period,
        z,
    )
...
def wave_stokes_drift_parameterised(wind, fetch):
    ...
    if __stokes_coefficients__ is None:
        ...
        for f in ["5000", "25000", "50000"]:
            c[f] = np.polyfit(range(len(Wf[f])), Wf[f], n[f])
...
def wave_significant_height_parameterised(wind, fetch):
    ...
    if __stokes_hs_coefficients__ is None:
        c = {}
        for f in ["5000", "25000", "50000"]:
            c[f] = np.polyfit(range(len(Sw[f])), Sw[f], 1)
        __stokes_hs_coefficients__ = c

    windspeed = np.sqrt(wind[0] ** 2 + wind[1] ** 2)
    windspeed[windspeed > 30] = 30
    wave_significant_height = np.polyval(__stokes_hs_coefficients__[fetch], windspeed)
...
def wave_radiation_force(
    ...
    F_wave_x = (
        ...
        * iceb_length
        * (wave_height / 2) ** 2
        * np.sin(np.deg2rad(wave_direction))
    )
    F_wave_y = (
        0.5
        * rho_water
        ...
        * iceb_length
        * (wave_height / 2) ** 2
        * np.cos(np.deg2rad(wave_direction))
    )
...
class PhysicsMethods:
    ...
    def sea_water_density(T=10.0, S=35.0):
        ...
        R1 = (
            (((6.536332e-09 * T - 1.120083e-06) * T + 1.001685e-04) * T - 9.095290e-03)
            * T
            + 6.793952e-02
        ) * T - 28.263737
        ...
        R2 = (
            ((5.3875e-09 * T - 8.2467e-07) * T + 7.6438e-05) * T - 4.0899e-03
        ) * T + 8.24493e-01
    ...
    def skillscore_trajectory(
        self,
        lon_obs,
        lat_obs,
        time_obs,
        duration=None,
        max_time_offset=timedelta(seconds=60),
        ...
        **kwargs : further arguments for skillscore method (e.g. tolerance_threshold)
        ...
        if timesteps_obs.min() != timesteps_obs.max():
            raise ValueError(
                "Time step of trajectory is variable: %s to %s"
                % (timesteps_obs.min(), timesteps_obs.max())
            )
        if timesteps_obs.min() != self.time_step_output:
            raise ValueError(
                "Time step of trajectory is %s, but time step of simulation is %s."
        ...
        for i in range(self.num_elements_total()):
            ...
            skillscore[i] = skillscore_liu_weissberg(
                lon_obs[indo],
                lat_obs[indo],
                lon_model[0:length],
                lat_model[0:length],
    ...
    def advect_ocean_current(self, factor=1):
        ...
        if cdfmin != 1 or cdfmax != 1:
            if cdfmin == cdfmax:
                ...
                logger.debug(
                    "Using currrent drift factor between %s and %s" % (cdfmin, cdfmax)
                )
        factor = factor * cdf
        # Runge-Kutta scheme
        if self.get_config("drift:advection_scheme")[0:11] == "runge-kutta":
            ...
            dist = speed * self.time_step.total_seconds() * 0.5
            geod = pyproj.Geod(ellps="WGS84")
            mid_lon, mid_lat, dummy = geod.fwd(
                self.elements.lon, self.elements.lat, az, dist, radians=False
            )
            ...
            if self.get_config("drift:advection_scheme") == "runge-kutta4":
                ...
                dist2 = speed2 * self.time_step.total_seconds() * 0.5
                lon2, lat2, dummy = geod.fwd(
                    self.elements.lon, self.elements.lat, az2, dist2, radians=False
                )
                ...
                dist3 = speed3 * self.time_step.total_seconds() * 0.5
                lon3, lat3, dummy = geod.fwd(
                    self.elements.lon, self.elements.lat, az3, dist3, radians=False
                )
    ...
    def advect_with_sea_ice(self, factor=1):
        if hasattr(self.environment, "sea_ice_x_velocity"):
            ...
            ice_velocity_x = (
                self.environment.x_sea_water_velocity + 0.015 * self.environment.x_wind
            )
            ice_velocity_y = (
                self.environment.y_sea_water_velocity + 0.015 * self.environment.y_wind
            )
            self.update_positions(factor * ice_velocity_x, factor * ice_velocity_y)
    ...
    def advect_wind(self, factor=1):
        ...
        if surface.sum() == 0:
            if surface_only is True:
                logger.debug(
                    "All elements are below surface, no wind-induced shear drift"
                )
            else:
                logger.debug(
                    "All elements are below %fm, no wind-induced shear drift"
                    % wind_drift_depth[0]
                )
            return
        ...
        speed = np.sqrt(
            x_wind[surface] * x_wind[surface] + y_wind[surface] * y_wind[surface]
        )
        ...
        if surface_only is True:
            logger.debug(
                "Advecting %s of %i elements at surface with "
            ...
            logger.debug(
                "Advecting %s of %i elements above %.3fm with "
                "wind-sheared ocean current (%f m/s - %f m/s)"
                % (
                    np.sum(surface),
                    self.num_elements_active(),
    ...
    def stokes_drift(self, factor=1):
        ...
        if np.max(np.array(wave_height)) == 0:
            logger.debug(
                "Stokes drift is available, but not Hs: using Hs=1 for Stokes profile"
            )
            wave_height = 1
        if np.max(np.array(wave_period)) == 0:
            logger.debug(
                "Stokes drift is available, but not Tp: using Tp=8 for Stokes profile"
            )
            wave_period = 8
        ...
        if s.min() == s.max():
            ...
            logger.debug(
                "Advecting with Stokes drift (%s to %s m/s)" % (s.min(), s.max())
            )
    ...
    def calculate_missing_environment_variables(self):
        ...
        if (
            hasattr(self.environment, "sea_surface_wave_significant_height")
            and self.environment.sea_surface_wave_significant_height.max() == 0
        ):
            Hs = self.significant_wave_height()
            logger.debug(
                "Calculating Hs from wind, min: %f, mean: %f, max: %f"
        ...
        if (
            ...
            and self.environment.sea_surface_wave_mean_period_from_variance_spectral_density_second_frequency_moment.max()
            == 0
        ):
            wave_period = self.wave_period()
            logger.debug(
                "Calculating wave period from wind, min: %f, mean: %f, max: %f"
                % (wave_period.min(), wave_period.mean(), wave_period.max())
            )

    def wind_speed(self):
        return np.sqrt(self.environment.x_wind**2 + self.environment.y_wind**2)

    def current_speed(self):
        return np.sqrt(
            self.environment.x_sea_water_velocity**2
    ...
    def wave_period(self):
        if (
            ...
            # prefer using Tm02:
            T = (
                self.environment.sea_surface_wave_mean_period_from_variance_spectral_density_second_frequency_moment.copy()
            )
            ...
            and self.environment.sea_surface_wave_period_at_variance_spectral_density_maximum.max()
            > 0
        ):
            # alternatively use Tp
            T = (
                self.environment.sea_surface_wave_period_at_variance_spectral_density_maximum.copy()
            )
            ...
            self.environment.sea_surface_wave_mean_period_from_variance_spectral_density_second_frequency_moment = (
                T
            )

        # print '\n T %s \n' % str(T.mean())
        if T.min() == 0:
            logger.warning("Zero wave period found - " "replacing with mean")
            T[T == 0] = np.mean(T[T > 0])
    ...
    def wave_damping_coefficient(self):
        ...
        return (10e-5) * omega * np.power(self.wave_energy(), 0.25)
    ...
    def sea_floor_depth(self):
        ...
        if hasattr(self, "environment") and hasattr(
            self.environment, "sea_floor_depth_below_sea_level"
        ):
            if (
                len(self.environment.sea_floor_depth_below_sea_level)
                == self.num_elements_active()
            ):
        ...
        if "sea_floor_depth" not in locals():
            env, env_profiles, missing = self.env.get_environment(
                ...
                z=0 * self.elements.lon,
                profiles=None,
            )
            sea_floor_depth = env["sea_floor_depth_below_sea_level"].astype("float32")
    ...
    def sea_surface_height(self):
        ...
        if hasattr(self, "environment") and hasattr(
            ...
            if len(self.environment.sea_surface_height) == self.num_elements_active():
        ...
        if "sea_surface_height" not in locals():
            ...
            sea_surface_height = env["sea_surface_height"].astype("float32")
...
def declination(time):
    ...
    declination = np.arcsin(
        ...
        * np.cos(
            np.radians(
                (360.0 / 365.24) * (day_of_year + 10)
                + (360.0 / np.pi)
                * 0.0167
                * np.sin(np.radians((360.0 / 365.24) * (day_of_year - 2)))
            )
        )
    )
...
def equation_of_time(time):
    ...
    gamma = 2 * np.pi / 365.0 * (day_of_year - 1.0 + (hour - 12.0) / 24.0)
    eqtime = 229.18 * (
        0.000075
