...
class OpenDriftSimulation(PhysicsMethods, Timeable, Configurable):
    ...
    def __init__(
        ...
        self.env = Environment(
            self.required_variables, self.required_profiles_z_range, self._config
        )
        ...
        self._add_config(
            {
                ...
                "general:time_step_output_minutes": {
                    ...
                    "units": "minutes",
                    "level": CONFIG_LEVEL_BASIC,
                    "description": "Output time step, i.e. the interval at which output is saved. This must be larger than "
        ...
        for p in self.ElementType.variables:
            ...
            c["seed:%s" % p] = {
                ...
                "description": (
                    v["description"]
                    if "description" in v
                    else "Seeding value of %s" % p
                ),
                "level": v["level"] if "level" in v else CONFIG_LEVEL_ADVANCED,
            }
        self._add_config(c)
        ...
        logger.info(
            "OpenDriftSimulation initialised (version %s)"
    ...
    @require_mode(
        mode=Mode.Config, error="Cannot set config after elements have been seeded"
    )
        ...
        if key == "seed:oil_type" and self.num_elements_scheduled() > 0:
            if value != self.get_config("seed:oil_type"):
                raise WrongMode(
                    Mode.Config,
                    self.mode,
                    msg=f"Cannot change the oil type after elements have been seeded: {self.get_config('seed:oil_type')} -> {value}",
                )
    ...
    def store_previous_variables(self):
        ...
        for var in self.store_previous:
            ...
            self.environment_previous[var][undefined] = getattr(self.environment, var)[
                undefined
            ]
    ...
    def interact_with_coastline(self, final=False):
        ...
        if final is True:  # Get land_binary_mask for final location
            en, en_prof, missing = self.env.get_environment(
                ["land_binary_mask"],
                self.time,
                self.elements.lon,
                self.elements.lat,
        ...
        if i == "stranding":  # Deactivate elements on land, but not in air
            ...
            if len(on_land) == 0:
                ...
                logger.debug(
                    "%s elements hit coastline, " "moving back to water" % len(on_land)
                )
                on_land_ID = self.elements.ID[on_land]
                self.elements.lon[on_land] = np.copy(self.previous_lon[on_land_ID - 1])
                self.elements.lat[on_land] = np.copy(self.previous_lat[on_land_ID - 1])
    ...
    def interact_with_seafloor(self):
        ...
        if i == "lift_to_seafloor":
            ...
            self.deactivate_elements(
                self.elements.z < -sea_floor_depth, reason="seafloor"
            )
            self.elements.z[below] = -sea_floor_depth[below]
        elif i == "previous":  # Go back to previous position (in water)
            logger.warning("%s elements hit seafloor, " "moving back " % len(below))
            below_ID = self.elements.ID[below]
            self.elements.lon[below] = np.copy(self.previous_lon[below_ID - 1])
            self.elements.lat[below] = np.copy(self.previous_lat[below_ID - 1])
    ...
    def test_data_folder(self):
        ...
        return (
            os.path.abspath(
                os.path.join(
                    os.path.dirname(opendrift.__file__), "..", "tests", "test_data"
                )
            )
            + os.path.sep
        )
    ...
    def schedule_elements(self, elements, time):
        ...
        if not hasattr(self, "elements_scheduled"):
            ...
            elements.ID = np.arange(
                self.num_elements_scheduled() + 1,
                self.num_elements_scheduled() + 1 + len(elements),
            )  # Increase ID successively
    ...
    def release_elements(self):
        ...
        if self.time_step.days >= 0:
            indices = (self.elements_scheduled_time >= self.time) & (
                self.elements_scheduled_time < self.time + self.time_step
            )
        else:
            indices = (self.elements_scheduled_time <= self.time) & (
                self.elements_scheduled_time > self.time + self.time_step
    ...
    def closest_ocean_points(self, lon, lat):
        ...
        if not "land_binary_mask" in self.env.priority_list:
            logger.info("No land reader added, " "making a temporary landmask reader")
        ...
        if land.max() == 0:
            ...
            return lon, lat
        logger.info(
            "Moving %i out of %i points from land to water"
        ...
        landgrid = o.env.get_environment(
            ["land_binary_mask"],
            lon=longrid,
            lat=latgrid,
            z=0 * longrid,
        ...
        tree = scipy.spatial.cKDTree(np.dstack([oceangridlons, oceangridlats])[0])
    ...
    @require_mode(mode=Mode.Ready)
    def seed_elements(
        self, lon, lat, time, radius=0, number=None, radius_type="gaussian", **kwargs
    ):
        ...
        if len(lon) > 1:
            if number is not None and number != len(lon):
                raise ValueError(
                    "Lon and lat have length %s, but number is %s" % (len(lon), number)
                )
            number = len(lon)
        else:
            if number is None:
        ...
        if len(time) != number and len(time) > 1:
            if len(time) == 2:  # start -> end
                ...
                raise ValueError(
                    "Time array has length %s, must be 1, 2 or %s" % (len(time), number)
                )
        ...
        if (
            ...
            if seafloor_constant is not None:
            ...
            elif ("sea_floor_depth_below_sea_level" in self.env.priority_list) or len(
                self.env._lazy_readers()
            ):
                if not hasattr(self, "time"):
                    self.time = time[0]
                env, env_profiles, missing = self.env.get_environment(
                ...
                raise ValueError(
                    "A reader providing the variable "
            ...
            if len(kwargs["z"]) > 8 and kwargs["z"][8] == "+":
                ...
                logger.info(
                    "Seeding elements %f meters above seafloor" % meters_above_seafloor
                )
            else:
    ...
    def seed_cone(self, lon, lat, time, radius=0, number=None, **kwargs):
        ...
        if len(lon) != len(lat):
            ...
            raise ValueError(
                "Lon and lat must have length 1 or 2, given length is %s" % (len(lon))
            )
        elif len(lon) == 1:
            lon = lon * np.ones(number)
            lat = lat * np.ones(number)
            ...
            conelonlats = geod.npts(
                lon[0], lat[0], lon[1], lat[1], number, radians=False
            )
        ...
        geo = geojson.LineString(
            [(float(lonin[0]), float(latin[0])), (float(lonin[1]), float(latin[1]))]
        )
        seed_defaults = self.get_configspec("seed")
        default_seed = {
            k.split(":")[-1]: seed_defaults[k]["value"] for k in seed_defaults
        }
        ...
        self.seed_elements(
            lon=lon, lat=lat, time=time, radius=radius, number=number, **kwargs
        )
    ...
    def seed_repeated_segment(
        ...
        **kwargs,
        ...
        numtimes = int(
            (end_time - start_time).total_seconds() / time_interval.total_seconds() + 1
        )
        ...
        s_lonlats = geod.npts(
            lons[0], lats[0], lons[1], lats[1], number_per_segment, radians=False
        )
        ...
        if total_number is not None:
            ...
            print(
                "Repeating the %d last points, to obtain %d elements"
    ...
    def seed_within_polygon(self, lons, lats, number=None, **kwargs):
        ...
        proj = pyproj.Proj(
            "+proj=aea +lat_1=%f +lat_2=%f +lat_0=%f "
    ...
    def seed_from_wkt(self, wkt, number=None, **kwargs):
        ...
        for i in range(0, geom.GetGeometryCount()):
            ...
            logger.info(
                "\tSeeding %s elements within polygon number %s"
                % (num_elements, str(i))
            )
            try:
            ...
            self.seed_within_polygon(
                lons=lons, lats=lats, number=num_elements, **kwargs
            )
            num_seeded += num_elements

    @require_mode(mode=Mode.Ready)
    def seed_from_shapefile(
        self, shapefile, number, layername=None, featurenum=None, **kwargs
    ):
        ...
        if "timeformat" in kwargs:
            ...
            kwargs["time"] = datetime.strptime(
                os.path.basename(shapefile), kwargs["timeformat"]
            )
        ...
        for layer in s:
            if layername is not None and layer.GetName() != layername:
                ...
                logger.info(
                    "Seeding for layer: %s (%s features)"
            ...
            if max(featurenum) > layer.GetFeatureCount():
                raise ValueError("Only %s features in layer." % layer.GetFeatureCount())
    ...
    def seed_letters(self, text, lon, lat, time, number, scale=1.2):
        ...
        pol = matplotlib.textpath.TextPath((lon, lat), text, size=1 * scale, prop=fp)
        ...
        for p in po:
            self.seed_within_polygon(
                lons=p[:, 0], lats=p[:, 1], number=number, time=time
            )
    ...
    def horizontal_diffusion(self):
        ...
        x_vel = (
            ...
            * np.sqrt(2 * D / dt)
            * np.random.normal(scale=1, size=self.num_elements_active())
        )
        y_vel = (
            self.elements.moving
            * np.sqrt(2 * D / dt)
    ...
    def deactivate_elements(self, indices, reason="deactivated"):
        ...
        self.elements.status[indices & (self.elements.status == 0)] = reason_number
        self.elements.moving[indices] = 0
        logger.debug(
            "%s elements scheduled for deactivation (%s)" % (np.sum(indices), reason)
        )
        logger.debug(
            "\t(z: %f to %f)"
    ...
    def remove_deactivated_elements(self):
        ...
        if hasattr(self, "environment"):
            ...
            logger.debug("Removed %i values from environment." % (np.sum(indices)))
        if (
            hasattr(self, "environment_profiles")
            and self.environment_profiles is not None
        ):
            ...
            logger.debug(
                "Removed %i values from environment_profiles." % (np.sum(indices))
            )
            # if self.num_elements_active() == 0:
    ...
    def run(
        ...
        if time_step_ratio < 1:
            raise ValueError(
                "Output time step must be equal or larger "
                "than calculation time step."
            )
        if not time_step_ratio.is_integer():
            raise ValueError(
                "Ratio of calculation and output time steps "
                "must be an integer - given ratio is %s" % time_step_ratio
            )
        ########################
        # Simulation duration
        ########################
        ...
        if (
            (duration is not None and end_time is not None)
            or (duration is not None and steps is not None)
            or (steps is not None and end_time is not None)
        ):
            raise ValueError(
                'Only one of "steps", "duration" and "end_time" '
        ...
        if duration is None and end_time is None:
            if steps is not None:
                ...
                for reader in self.env.readers.values():
                    ...
                    logger.info(
                        "Duration, steps or end time not specified, "
                        "running until end of first reader: %s" % (end_time)
                    )
        if duration is None:
        ...
        self.expected_steps_output = (
            duration.total_seconds() / self.time_step_output.total_seconds() + 1
        )  # Includes start and end
        self.expected_steps_calculation = (
            duration.total_seconds() / self.time_step.total_seconds()
        )
        ...
        self.expected_end_time = (
            self.start_time + self.expected_steps_calculation * self.time_step
        )
        ...
        deltalat = max_distance / 111000.0
        ...
        simulation_extent = [
            ...
            np.minimum(360, self.elements_scheduled.lon.max() + deltalon),
        ...
        if self.time_step.days < 0:
            ...
            logger.info(
                "Backwards simulation, starting at " "time of last seeded element"
            )
            self.time = self.elements_scheduled_time.max()
            # Flipping ID array, so that lowest IDs are released first
            self.elements_scheduled.ID = np.flipud(self.elements_scheduled.ID)
        else:
        ...
        if self.export_variables is not None:
            history_dtype_fields = [
                f for f in history_dtype_fields if f[0] in self.export_variables
            ]
        ...
        self.history = np.ma.array(
            np.zeros((len(self.elements_scheduled), self.export_buffer_length)),
            dtype=history_dtype,
        )
        ...
        for i in range(self.expected_steps_calculation):
            try:
                ...
                logger.debug("===================================" * 2)
                logger.info(
                    "%s - step %i of %i - %i active elements "
                    "(%i deactivated)"
                    % (
                        self.time,
                        self.steps_calculation + 1,
                ...
                logger.debug("%s elements scheduled." % self.num_elements_scheduled())
                logger.debug("===================================" * 2)
                ...
                logger.debug(
                    "%s active elements (%s deactivated)"
                    % (self.num_elements_active(), self.num_elements_deactivated())
                )
                # Updating time
                if self.time is not None:
                    self.time = self.time + self.time_step
        ...
        for var in self.required_variables:
            ...
            if var not in self.env.priority_list:
                ...
                if (
                    readers[0].startswith("constant_reader")
                    and var in self.env.readers[readers[0]]._parameter_value_map
                ):
                    self.add_metadata(
                        keyword,
    ...
    def state_to_buffer(self):
        ...
        steps_calculation_float = (
            ...
            * self.time_step.total_seconds()
            / self.time_step_output.total_seconds()
        ) + 1
        ...
        # print(time_ind, self.steps_calculation, steps_calculation_float, "DEBUG")
        ...
        if (
            steps_calculation_float.is_integer()
            or self.time_step < timedelta(seconds=1)
            or final_time_step is True
        ):
            ...
            logger.debug(
                "Writing history for %s deactivated elements" % len(deactivated)
            )
        ...
        for i, var in enumerate(self.elements.variables):
            if self.export_variables is not None and var not in self.export_variables:
                continue
            ...
            self.history[var][ID_ind, time_ind] = getattr(self.elements, var)[
                element_ind
            ]
            if len(ID_ind) > 0:
        ...
        for i, var in enumerate(self.environment.dtype.names):
            if self.export_variables is not None and var not in self.export_variables:
                continue
            self.history[var][ID_ind, time_ind] = getattr(self.environment, var)[
                element_ind
            ]
            if len(ID_ind) > 0:
                newmin = np.min(self.history[var][ID_ind, time_ind])
        ...
        if (self.outfile is not None) and (
            (self.steps_output - self.steps_exported) == self.export_buffer_length
        ):
            self.io_write_buffer()
    ...
    def set_up_map(
        ...
        **kwargs,
        ...
        if corners is not None:  # User provided map corners
            ...
            if (
                "compare_lonmin" in kwargs
            ):  # checking min/max lon/lat of other simulations
        ...
        ax.set_extent(
            [lonmin, lonmax, latmin, latmax], crs=ccrs.PlateCarree(globe=globe)
        )
        ...
        if not hide_landmask:
            if (
                "land_binary_mask" in self.env.priority_list
                and self.env.priority_list["land_binary_mask"][0] == "shape"
            ):
                logger.debug("Using custom shapes for plotting land..")
                ax.add_geometries(
                    self.env.readers["shape"].polys,
                    ccrs.PlateCarree(globe=globe),
                    facecolor=land_color,
                    edgecolor="black",
                )
            else:
                reader_global_landmask.plot_land(
                    ax,
                    lonmin,
                    latmin,
                    lonmax,
    ...
    def animation(
        ...
        **kwargs,
        ...
        if (
            ...
            and self.num_elements_total() == 0
            and not hasattr(self, "ds")
        ):
        ...
        if density is True:  # Get density arrays
            if hasattr(self, "ds"):  # opened with Xarray
                ...
                H, H_submerged, H_stranded, lon_array, lat_array = (
                    self.get_density_array(
                        pixelsize_m=density_pixelsize_m, weight=density_weight
                    )
                )
                H = H + H_submerged + H_stranded

        # Find map coordinates and plot points with empty data
        fig, ax, crs, x, y, index_of_first, index_of_last = self.set_up_map(
            buffer=buffer,
            corners=corners,
            lscale=lscale,
            fast=fast,
            hide_landmask=hide_landmask,
            **kwargs,
        )
        ...
        def plot_timestep(i):
            ...
            if background is not None:
                ...
                if isinstance(background, xr.DataArray):
                    ...
                    map_x, map_y, scalar, u_component, v_component = (
                        self.get_map_background(ax, background, crs, time=times[i])
                    )
                ...
                if type(background) is list:
                    ret.append(bg_quiv)
                    bg_quiv.set_UVC(
                        u_component[::skip, ::skip], v_component[::skip, ::skip]
                    )
            ...
            if show_elements is True:
                points.set_offsets(
                    np.c_[x[i, range(x.shape[1])], y[i, range(x.shape[1])]]
                )
                ...
                if color is not False:  # Update colors
                    ...
                    if isinstance(color, str) or hasattr(color, "__len__"):
                        points_deactivated.set_array(
                            colorarray_deactivated[index_of_last_deactivated < i]
                        )

            if drifter is not None:
                for drnum, dr in enumerate(drifter):
                    drifter_pos[drnum].set_offsets(np.c_[dr["x"][i], dr["y"][i]])
            ...
            if show_elements is True:
                if compare is not None:
                    for cd in compare_list:
                        ...
                        cd["points_other_deactivated"].set_offsets(
                            np.c_[
                                cd["x_other_deactive"][
        ...
        if color is not False and show_elements is True:
            if isinstance(color, str):
                ...
                colorarray_deactivated = self.get_property(color)[0][
        ...
        if background is not None:
            if isinstance(background, xr.DataArray):
                ...
                map_x, map_y, scalar, u_component, v_component = (
                    self.get_map_background(ax, background, crs, time=self.start_time)
                )
            bg = ax.pcolormesh(
                map_x,
                map_y,
        ...
        if show_elements is True:
            index_of_last_deactivated = index_of_last[self.elements_deactivated.ID - 1]
        if legend is None:
            legend = [""]
        ...
        x_deactive, y_deactive = (
            self.elements_deactivated.lon,
        ...
        if compare is not None:
            for cn, cd in enumerate(compare_list):
                ...
                # Plot deactivated elements, with transparency
                cd["points_other_deactivated"] = ax.scatter(
                    [],
                    [],
                    alpha=0.3,
                    zorder=9,
        ...
        self.__save_or_plot_animation__(
            plt.gcf(), plot_timestep, filename, frames, fps, interval=50, blit=blit
        )

        logger.info("Time to make animation: %s" % (datetime.now() - start_time))

    def __save_or_plot_animation__(
        self, figure, plot_timestep, filename, frames, fps, interval, blit
    ):

        if filename is not None or "sphinx_gallery" in sys.modules:
            logger.debug("Saving animation..")
            self.__save_animation__(
                figure,
                plot_timestep,
                filename,
                frames=frames,
            ...
            anim = animation.FuncAnimation(
                figure, plot_timestep, blit=blit, frames=frames, interval=interval
            )
            try:
                plt.show()
    ...
    def animation_profile(
        ...
        def plot_timestep(i):
            ...
            if PlotColors:
                points.set_offsets(
                    np.array([x[range(x.shape[0]), i].T, z[range(x.shape[0]), i].T]).T
                )
                points.set_array(colorarray[:, i])
            else:
                points.set_data(x[range(x.shape[0]), i], z[range(x.shape[0]), i])
            ...
            if compare is not None:
                points_other.set_data(
                    x_other[range(x_other.shape[0]), i],
        ...
        # Set up plot
        index_of_first, index_of_last = self.index_of_activation_and_deactivation()
        z = self.get_property("z")[0].T
        x = self.get_property("lon")[0].T

        # seafloor_depth = \
        #    -self.get_property('sea_floor_depth_below_sea_level')[0].T
        fig = plt.figure(figsize=(10, 6.0))  # Suitable aspect ratio
        ax = fig.gca()
        ...
        if PlotColors:
            ...
            points = plt.plot([], [], ".k", label=legend[0], markersize=markersize)[0]

        # Plot deactivated elements, with transparency
        points_deactivated = plt.plot([], [], ".k", alpha=0.3)[0]
        ...
        if compare is not None:
            ...
            # Plot deactivated elements, with transparency
            points_other_deactivated = plt.plot([], [], ".r", alpha=0.3)[0]
            ...
            index_of_last_deactivated_other = index_of_last_other[
                other.elements_deactivated.ID - 1
            ]
        ...
        # Set figure limits
        sky = (zmax - zmin) * 0.1  # Sky height is 10% of water depth
        plt.xlim([xmin, xmax])
        plt.ylim([zmin, sky])
        ax.add_patch(plt.Rectangle((xmin, 0), xmax - xmin, sky, color="lightsteelblue"))
        ax.add_patch(
            plt.Rectangle((xmin, zmin), xmax - xmin, -zmin, color="cornflowerblue")
        )
    ...
    def _get_comparison_xy_for_plots(self, compare):
        ...
        for cn, comp in enumerate(compare):
            ...
            cd["x_other"], cd["y_other"] = (
                other.history["lon"].copy(),
                other.history["lat"].copy(),
            )
            ...
            cd["index_of_last_deactivated_other"] = cd["index_of_last_other"][
                other.elements_deactivated.ID - 1
            ]
    ...
    def plot(
        ...
        **kwargs,
        ...
        if lalpha is None:
            ...
            max_elements = 5000.0
            alpha = min_alpha ** (
                2 * (self.num_elements_total() - 1) / (max_elements - 1)
            )
        ...
        if self.history is not None and linewidth != 0 and show_trajectories is True:
            ...
            if linecolor is None or is_color_like(linecolor) is True:
                ...
                if compare is not None and legend is not None:
                    if legend is True:
                        if hasattr(compare, "len"):
                            ...
                            numleg = 2
                        legend = ["Simulation %d" % (i + 1) for i in range(numleg)]
                    ax.plot(
                        x[:, 0],
                        y[:, 0],
                        color=linecolor,
                ...
                for i in range(x.shape[1]):
                    ...
                    points = np.array([x[vind, i].T, y[vind, i].T]).T.reshape(-1, 1, 2)
        ...
        if compare is None:
            ...
            label_active = "active (%i)" % (
                x.shape[1] - self.num_elements_deactivated()
            )
        ...
        if show_elements is True:
            if show_initial is True:
                ax.scatter(
                    x[index_of_first, range(x.shape[1])],
            ...
            ax.scatter(
                x[index_of_last, range(x.shape[1])],
        ...
        if compare is not None:
            for i, c in enumerate(cd):
                ...
                ax.scatter(
                    c["x_other"][
                        range(c["x_other"].shape[0]), c["index_of_last_other"]
                    ],
                    c["y_other"][
                        range(c["y_other"].shape[0]), c["index_of_last_other"]
                    ],
                    s=markersize,
                    zorder=3,
                    edgecolor=markercolor,
            ...
            if show_scalar is True:
                if contourlines is False:
                    ...
                    if contourlines is True:
                        CS = ax.contour(
                            map_x, map_y, scalar, colors="gray", transform=gcrs
                        )
                    else:
        ...
        if filename is not None:
            ...
            logger.info("Time to make plot: " + str(datetime.now() - start_time))
        else:
            if show is True:
                plt.show()
    ...
    def _figure_title(self):
        if self._substance_name() is None:
            ...
            return (
                "OpenDrift - " + type(self).__name__ + " (%s)" % self._substance_name()
            )
    ...
    def _plot_drifter(self, ax, gcrs, drifter):
        ...
        x, y = (np.atleast_1d(drifter["lon"])[i], np.atleast_1d(drifter["lat"])[i])
    ...
    def get_map_background(self, ax, background, crs, time=None):
        ...
        for readerName in self.env.readers:
            ...
            if variable in reader.variables:
                if (
                    time is None
                    or reader.start_time is None
                    or (time >= reader.start_time and time <= reader.end_time)
    ...
    def get_histogram(self, pixelsize_m, **kwargs):
        ...
        for om in origin_marker:
            ...
            h = histogram(
                self.ds.lon.where(self.ds.origin_marker == om),
    ...
    def get_density_array(self, pixelsize_m, weight=None):
        ...
        times = self.get_time_array()[0]
        deltalat = pixelsize_m / 111000.0  # m to degrees
        deltalon = deltalat / np.cos(np.radians((np.nanmin(lat) + np.nanmax(lat)) / 2))
        lat_array = np.arange(
            np.nanmin(lat) - deltalat, np.nanmax(lat) + deltalat, deltalat
        )
        lon_array = np.arange(
            np.nanmin(lon) - deltalat, np.nanmax(lon) + deltalon, deltalon
        )
        ...
        lat[z < 0] = 1000
        H = np.zeros(
            (len(times), len(lon_array) - 1, len(lat_array) - 1)
        )  # .astype(int)
        H_submerged = H.copy()
        H_stranded = H.copy()
        try:
        ...
        for i in range(len(times)):
            if weight is not None:
                ...
                weights = None
            H[i, :, :], dummy, dummy = np.histogram2d(
                lon[i, :], lat[i, :], weights=weights, bins=bins
            )
            H_submerged[i, :, :], dummy, dummy = np.histogram2d(
                lon_submerged[i, :], lat_submerged[i, :], weights=weights, bins=bins
            )
            if contains_stranded is True:
                H_stranded[i, :, :], dummy, dummy = np.histogram2d(
                    lon_stranded[i, :], lat_stranded[i, :], weights=weights, bins=bins
                )

        return H, H_submerged, H_stranded, lon_array, lat_array

    def get_density_array_proj(
        self,
        pixelsize_m,
        density_proj=None,
        llcrnrlon=None,
        ...
        x[z < 0] = outsidex
        y[z < 0] = outsidey
        H = np.zeros((len(times), len(x_array) - 1, len(y_array) - 1))  # .astype(int)
        H_submerged = H.copy()
        H_stranded = H.copy()
        try:
            strandnum = self.status_categories.index("stranded")
        ...
        for i in range(len(times)):
            if weight is not None:
                ...
                weights = None
            H[i, :, :], dummy, dummy = np.histogram2d(
                x[i, :], y[i, :], weights=weights, bins=bins
            )
            H_submerged[i, :, :], dummy, dummy = np.histogram2d(
                x_submerged[i, :], y_submerged[i, :], weights=weights, bins=bins
            )
            if contains_stranded is True:
                H_stranded[i, :, :], dummy, dummy = np.histogram2d(
                    x_stranded[i, :], y_stranded[i, :], weights=weights, bins=bins
                )
    ...
    def get_residence_time(self, pixelsize_m):
        H, H_sub, H_str, lon_array, lat_array = self.get_density_array(pixelsize_m)
    ...
    def write_netcdf_density_map(self, filename, pixelsize_m="auto"):
        ...
        H, H_submerged, H_stranded, lon_array, lat_array = self.get_density_array(
            pixelsize_m
        )
        ...
        nc.variables["projection_lonlat"].grid_mapping_name = "latitude_longitude"
        nc.variables["projection_lonlat"].earth_radius = 6371229.0
        nc.variables["projection_lonlat"].proj4 = "+proj=longlat +a=6371229 +no_defs"
        # Coordinates
    ...
    def write_netcdf_density_map_proj(
        self,
        filename,
        pixelsize_m="auto",
        density_proj=None,
        ...
        H, H_submerged, H_stranded, lon_array, lat_array = self.get_density_array_proj(
        ...
        lon_array = (lon_array[:-1, :-1] + lon_array[1:, 1:]) / 2.0
        lat_array = (lat_array[:-1, :-1] + lat_array[1:, 1:]) / 2.0
    ...
    def write_geotiff(self, filename, pixelsize_km=0.2):
        ...
        ilat[ilat.mask] = 0
        status[ilon.mask] = 0
        image = np.zeros((len(times), len(lon_array), len(lat_array))).astype(int)
        geotransform = [lon_array.min(), deltalon, 0, lat_array.max(), 0, -deltalat]
    ...
    def get_time_array(self):
        ...
        time_array = [self.start_time + td * i for i in range(self.steps_output)]
    ...
    def plot_environment(self, filename=None, ax=None, show=True):
        ...
        time = np.array([t.total_seconds() / 3600.0 for t in time_relative])
    ...
    def plot_property(self, prop, filename=None, mean=False):
        ...
        times = [
            start_time + n * self.time_step_output for n in range(self.steps_output)
        ]
    ...
    def get_trajectory_lengths(self):
        ...
        geod = pyproj.Geod(ellps="WGS84")
        a1, a2, distances = geod.inv(
            lons[0:-1, :], lats[0:-1, :], lons[1::, :], lats[1::, :]
        )
    ...
    def update_positions(self, x_vel, y_vel):
        ...
        if (
            (self.elements.lon.min() < -180)
            or (self.elements.lon.min() > 360)
            or (self.elements.lat.min() < -90)
            or (self.elements.lat.max() > 90)
        ):
    ...
    def add_halo_readers(self):
        ...
        self.add_readers_from_file(
            self.test_data_folder() + "../../opendrift/scripts/data_sources.txt"
        )
    ...
    def __save_animation__(
        self, fig, plot_timestep, filename, frames, fps, blit, interval
    ):
        ...
        if str(filename)[-4:] == ".gif":
            ...
            anim = animation.FuncAnimation(
                fig, plot_timestep, blit=blit, frames=frames, interval=interval
            )
            anim.save(filename)
        ...
        logger.debug("Time to save animation: %s" % (datetime.now() - start_time))
    ...
    def calculate_ftle(
        ...
        for i, t in enumerate(time):
            ...
            if RLCS is True:
                ...
                f_x1, f_y1 = proj(
                    o.history["lon"].T[-1].reshape(X.shape),
            ...
            if ALCS is True:
                o = self.clone()
                o.seed_elements(lons.ravel(), lats.ravel(), time=t + duration, z=z)
                o.run(duration=duration, time_step=-time_step)
                b_x1, b_y1 = proj(
                    o.history["lon"].T[-1].reshape(X.shape),
                    o.history["lat"].T[-1].reshape(X.shape),
