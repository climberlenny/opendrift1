...
class Reader(StructuredReader, BaseReader):
    ...
    def __init__(
        ...
        if isinstance(filename, xr.Dataset):
            ...
            if zarr_storage_options is not None:
                self.Dataset = xr.open_zarr(
                    filename, storage_options=zarr_storage_options
                )
                if name is None:
                    self.name = filename
                else:
        ...
        if (
            "ocean_time" in self.Dataset.dims
            and "eta_u" in self.Dataset.dims
            and "eta_rho" in self.Dataset.dims
        ):
            raise ValueError(
                "This seems to be a ROMS native file, should use ROMS native reader instead"
            )
        ...
        for var_name in self.Dataset.variables:
            ...
            if self.proj4 is None:
                if "grid_mapping_name" in var.attrs:
                    logger.debug(
                        ("Parsing CF grid mapping dictionary:" " " + str(var.attrs))
                    )
            ...
            standard_name = (
                var.attrs["standard_name"] if "standard_name" in var.attrs else ""
            )
            ...
            CoordinateAxisType = (
                var.attrs["CoordinateAxisType"]
                if "CoordinateAxisType" in var.attrs
                else ""
            )
            if (
                standard_name == "longitude"
                or CoordinateAxisType.lower() == "lon"
                or long_name.lower() == "longitude"
                or var_name.lower() in ["longitude", "lon"]
            ):
                lon_var_name = var_name
            if (
                standard_name == "latitude"
                or CoordinateAxisType.lower() == "lat"
                or long_name.lower() == "latitude"
                or var_name.lower() in ["latitude", "lat"]
            ):
                lat_var_name = var_name
            if (
                axis == "X"
                or standard_name == "projection_x_coordinate"
                or standard_name == "grid_longitude"
            ) and var.ndim == 1:
                self.xname = var_name
                if len(var.dims) == 1:
            ...
            if (
                axis == "Y"
                or standard_name == "projection_y_coordinate"
                or standard_name == "grid_latitude"
            ) and var.ndim == 1:
                self.yname = var_name
                if len(var.dims) == 1:
            ...
            if (standard_name == "depth" or axis == "Z") and var.ndim == 1:
                ...
                if var_data.ndim == 1:  # Earlier this was not a requirement above
                    if "positive" not in var.attrs or var.attrs["positive"] == "up":
                        self.z = var_data
                    else:
                        self.z = -var_data
            if standard_name == "time" or axis == "T" or var_name in ["time", "vtime"]:
                ...
                if isinstance(time[0], np.bytes_):
                    ...
                    self.times = [
                        datetime.fromisoformat(t.replace("Z", "")) for t in time
                    ]
                elif time.ndim == 2:
                    self.times = [
                        datetime.fromisoformat("".join(t).replace("Z", ""))
                        for t in time.astype(str)
                    ]
                else:
            ...
            if standard_name == "realization":
                if ensemble_member == None:
                    ...
                    logger.debug(
                        "%i ensemble members available" % len(self.realizations)
                    )
                if len(var.dims) == 1:
        ...
        if "x" in locals():
            if x[1] - x[0] == 1 and y[1] - y[0] == 1:
                logger.info(
                    "deltaX and deltaY are 1, interpreting dataset as unprojected"
                )
                projected = False
            else:
                projected = True
        if (
            "x" not in locals() or projected is False
        ):  # No x/y-coordinates were detected
            if lon_var_name is None:
                raise ValueError(
                    "No geospatial coordinates were detected, cannot geolocate dataset"
                )
            ...
            if lon_var.ndim == 1:
                logger.debug(
                    "Lon and lat are 1D arrays - using as projection coordinates"
                )
                x = lon_var.data
                y = lat_var.data
            ...
            if self.proj4 is None:
                logger.info(
                    "Grid coordinates are detected, but proj4 string not given: assuming latlong"
                )
                self.proj4 = "+proj=latlong"
        ...
        if "x" in locals() and "y" in locals():
            ...
            rel_delta_x = np.abs((rel_delta_x.max() - rel_delta_x.min()) / self.delta_x)
            rel_delta_y = y[1::] - y[0:-1]
            rel_delta_y = np.abs((rel_delta_y.max() - rel_delta_y.min()) / self.delta_y)
        ...
        if (
            self.proj4 is not None
            and "latlong" in self.proj4
    ...
    def get_variables(
        ...
        indx = (
            np.floor(np.abs(x - self.x[0]) / self.delta_x - clipped).astype(int)
            + clipped
        )
        indy = (
            np.floor(np.abs(y - self.y[0]) / self.delta_y - clipped).astype(int)
            + clipped
        )
        buffer = (
            self.buffer
        ...
        indy = np.arange(
            np.max([0, indy.min() - buffer]), np.min([indy.max() + buffer, self.numy])
        )
        ...
        if (
            ...
            and indx.min() < 0
            and indx.max() > 0
            and indx.max() < self.numx
        ):
            logger.debug(
                "Requested data block is not continuous in file"
            ...
            continuous = True
            indx = np.arange(np.max([0, indx.min()]), np.min([indx.max(), self.numx]))
        ...
        for par in requested_variables:
            if hasattr(self, "rotate_mapping") and par in self.rotate_mapping:
                logger.debug(
                    "Using %s to retrieve %s" % (self.rotate_mapping[par], par)
                )
                if par not in self.variable_mapping:
                    self.variable_mapping[par] = self.variable_mapping[
            ...
            if continuous is True:
                if True:  # new dynamic way
                    ...
                    subset = {
                        vdim: dimindices[dim]
                        for dim, vdim in self.dimensions.items()
                        if vdim in var.dims
                    }
            ...
            # The below should also be updated to dynamic subsetting
                ...
                if var.ndim == 2:
                    ...
                    left = var[indxTime, indz, indrealization, indy, indx_left]
                    right = var[indxTime, indz, indrealization, indy, indx_right]
            ...
            with np.errstate(invalid="ignore"):
                variables[par] = np.ma.masked_outside(variables[par], -30000, 30000)
            ...
            if ensemble_dim is not None:
                ...
                for ensemble_num in range(num_ensembles):
                    newvar[ensemble_num] = np.take(
                        variables[par], ensemble_num, ensemble_dim
                    )
        ...
        if self.projected is True:
            variables["x"] = self.Dataset.variables[self.xname][indx] * self.unitfactor
            variables["y"] = self.Dataset.variables[self.yname][indy] * self.unitfactor
        else:
            variables["x"] = indx
            variables["y"] = indy
        ...
        if hasattr(self, "rotate_mapping"):
            if self.y_is_north() is True:
                ...
                for vectorpair in vector_pairs_xy:
                    if (
                        vectorpair[0] in self.rotate_mapping
                        and vectorpair[0] in variables.keys()
                    ):
                        if self.proj.__class__.__name__ == "fakeproj":
                            logger.warning(
                                "Rotation from fakeproj is not yet implemented, skipping."
                            )
                            continue
                        logger.debug(
                            f"Rotating vector from east/north to xy orientation: {vectorpair[0:2]}"
                        )
                        variables[vectorpair[0]], variables[vectorpair[1]] = (
                            self.rotate_vectors(
                                lon,
                                lat,
                                variables[vectorpair[0]],
                                variables[vectorpair[1]],
        ...
        if hasattr(self, "shift_x"):
            ...
            if self.proj.crs.is_geographic:  # meters to degrees
                ...
                logger.info(
                    "Shifting x between %s and %s" % (shift_x.min(), shift_x.max())
                )
